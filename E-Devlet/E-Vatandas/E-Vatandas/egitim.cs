...
namespace E_Vatandas
{
    public partial class egitim : Form
    {
        ...
        public egitim(string vid)
        {
            InitializeComponent();
            SqlConnection conn1 = new SqlConnection("Data Source=.;Initial Catalog=e_vatandas;Integrated Security=True");
            conn1.Open();
            SqlCommand retrieve = new SqlCommand("SELECT * FROM egitim_bilgileri WHERE vid=@vid", conn1);
            retrieve.Parameters.AddWithValue("vid", vid);
            SqlDataReader read;
            read = retrieve.ExecuteReader();
            DataTable dt = new DataTable();
            dt.Columns.Add("Okul Adı");
            dt.Columns.Add("Eğitim Durumu");
            dt.Columns.Add("Mezuniyet Tarihi");
            dt.Columns.Add("Burs");
            dt.Columns.Add("Diploma Notu");
            dt.Columns.Add("Mezuniyet Durumu");
            while (read.Read())
            {
                DataRow row = dt.NewRow();
                row["Okul Adı"] = read["okul_adi"];
                row["Eğitim Durumu"] = read["egitim_durumu"];
                row["Mezuniyet Tarihi"] = read["mezuniyet_tarihi"];
                row["Burs"] = read["burs"];
                row["Diploma Notu"] = read["diploma_notu"];
                row["Mezuniyet Durumu"] = read["mezuniyet_durumu"];
                dt.Rows.Add(row);
            }
